...
export const Slide5_ModelTesting: React.FC = () => {
    ...
    const fitLabel =
        complexity < 3
            ? (language === 'zh' ? '欠拟合：模型过于简单' : 'Underfitting: model too simple')
            : complexity > 7
                ? (language === 'zh' ? '过拟合：模型过于复杂' : 'Overfitting: model too complex')
                : (language === 'zh' ? '拟合良好：复杂度适中' : 'Good fit: balanced complexity');
    const fitTone =
        complexity < 3
    ...
    const copy = {
        zh: {
            panel: `**模型测试**\n\n- 在模型从未见过的数据上进行测试。\n- 过拟合：训练表现高，但测试表现下降。\n- 欠拟合：训练与测试都低，因为模型过于简单。`,
            title: '模型复杂度变化下的训练/测试准确率',
            trainingAccuracy: '训练准确率',
            testAccuracy: '测试准确率',
            legendNote: '绿色：训练准确率，蓝色：测试准确率。',
        ...
        en: {
            ...
            title: 'Training vs test accuracy across model complexity',
            trainingAccuracy: 'Training accuracy',
            testAccuracy: 'Test accuracy',
            legendNote: 'Green: training accuracy, blue: test accuracy.',
            ...
            <ConceptStage>
                <div className="w-full h-full p-8 flex items-center gap-8">
                    ...
                    <div className="w-64 bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
                        <div className="text-xs uppercase text-gray-400">{text.modelComplexity}</div>
                        ...
                        <input
                            ...
                            aria-label={text.modelComplexity}
                        />

                        <div className="mt-6 text-xs uppercase text-emerald-500">{text.trainingAccuracy}</div>
                        <div className="text-lg font-semibold text-emerald-600 mt-1">
                            {(currentTrain * 100).toFixed(1)}%
                        </div>

                        <div className="mt-4 text-xs uppercase text-blue-500">{text.testAccuracy}</div>
